...
#define N 2
void ST7735_Line(int32_t y1, int32_t y2, int32_t color){	
	static int x;
	int32_t y_diff;
	y_diff = y2-y1;
	int32_t MotorSpeed = getCalculatedSpeed(getPeriod());
	int32_t graph_speed = (MotorSpeed*-2) + 158;
	
	if(y_diff == 0){
		ST7735_DrawPixel( x, y2, color);
		ST7735_DrawPixel( x, y2+1, color);
		ST7735_DrawPixel( x+1, y2, color);
		ST7735_DrawPixel( x+1, y2+1, color);
		ST7735_DrawPixel(x, graph_speed, 0);
		x++;
		if(x>=128) x = 0;
		ST7735_DrawFastVLine(x+2,30,128,ST7735_WHITE);
	}
	else{
		while (y1 != y2){
			if(y_diff>0) y1 = y1 + 1;
			...
			ST7735_DrawPixel( x, y1+1, color);
			ST7735_DrawPixel( x+1, y1, color);
			ST7735_DrawPixel( x+1, y1+1, color);
			ST7735_DrawPixel(x, graph_speed, 0);
			x++;
			if(x>=128) x = 0;
			ST7735_DrawFastVLine(x+2,30,128,ST7735_WHITE);
			MotorSpeed = getCalculatedSpeed(getPeriod());
			graph_speed = (MotorSpeed*-2) + 158;
		}
	}
}
...
void ST7735_PlotClear(int32_t ymin, int32_t ymax){
  ST7735_FillRect(0, 0, 128, 256, 0); // light grey
  if(ymax>ymin){
    Ymax = ymax;
    Ymin = ymin;
    Yrange = ymax-ymin;
...
// Outputs: none
void ST7735_PlotPointBlue(int32_t y){int32_t j;
  if(y<Ymin) y=Ymin;
  if(y>Ymax) y=Ymax;
...
// Inputs: y is the y coordinate of the point plotted
// Outputs: none
void ST7735_PlotPointBlack(int32_t y){int32_t j;
  if(y<Ymin) y=Ymin;
  if(y>Ymax) y=Ymax;
  // X goes from 0 to 127
  // j goes from 159 to 32
  ...
  ST7735_DrawPixel(X+1, j,   ST7735_BLACK);
  ST7735_DrawPixel(X,   j+1, ST7735_BLACK);
  ST7735_DrawPixel(X+1, j+1, ST7735_BLACK);
}
// *************** ST7735_PlotLine ********************
